#!/bin/zsh
# Forge lane of the harness. Adapted from the NeoForge lane; the loaders differ in how they are launched and in
# what they call their own API, and in nothing else that matters here.
# Original note: Same pass rule as the Fabric and Quilt lanes: the world starts loading, the game is
...
nf_run() {
  ...
  while [[ $added == 1 ]]; do
    added=0
    # The NeoForge lane resolves dependencies from a shelf of native 26.2 builds. Forge 26.2 is new enough that
    # almost nothing ships one, so the corpus is its own shelf: a dependency is ported alongside the mod that wants
    # it, the way the Fabric lane does it. The verdict is still about the mod under test alone.
    for bjar in ~/foxgrade-work/forge-corpus/*.jar; do
  ...
  /usr/bin/java -XstartOnFirstThread -Xmx3G -Djava.net.preferIPv6Addresses=system \
    ...
    -cp "$CP" net.minecraftforge.bootstrap.ForgeBootstrap \
    --launchTarget forge_client \
    --username FGTest --version $FORGEVER --gameDir "$PT" \
  ...
  mkdir -p $B/crashes-forge; for c in $PT/crash-reports/*.txt; do cp "$c" "$B/crashes-forge/$name--$(basename "$c")"; done
  ...
  mkdir -p $B/ports-forge; for pj in $PT/mods/*-fgport.jar; do cp "$pj" "$B/ports-forge/${name}--$(basename $pj)"; done
...
if [ $# -gt 0 ]; then
  ...
  for jar in ~/foxgrade-work/forge-corpus/*.jar; do nf_run "$(basename ${jar%%--*})" "$jar"; done
  echo "FORGE RUN DONE" >> $LEDGER
fi
