#!/usr/bin/env python3
"""Build the Forge test corpus the same way the NeoForge one was built: the most-used mods that really ship a
...
OUT = pathlib.Path.home() / "foxgrade-work/forge-corpus"
...
def main():
    ...
    while len(hits) < WANT * 4 and offset < 300:
        page = api("search", facets=[["categories:forge"], [f"versions:{SRC}"], ["project_type:mod"]],
