...
reset() {
  ...
  rm -f $PT/mods/retromod-input/*.jar $PT/mods/retromod-input/processed/*.jar $PT/retromod-input/*.jar $PT/retromod-input/processed/*.jar $PT/retromod-input/*.jar $PT/retromod-input/processed/*.jar
...
rm_run() {
  ...
  if [ "$v" = PASS ] && [ -n "$mainid" ] && ! grep -qE "^[[:space:]]*[-\\|]+[[:space:]]*${mainid}([_-][A-Za-z0-9]+)?[[:space:]]|^\\|[^|]*\\|[^|]*\\|[[:space:]]*${mainid}([_-][A-Za-z0-9]+)?[[:space:]]*\\|" "$B/log-rm-$name.log"; then v=HELD; fi
  pkill -f "gameDir $PT " 2>/dev/null; sleep 2
