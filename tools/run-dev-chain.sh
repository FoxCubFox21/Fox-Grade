...
export PT=$HOME/mc-porttest-dev
for slug in "$@"; do
  if [[ $slug == */* ]]; then jars=$slug; flags=-; slug=$(basename $slug .jar | sed 's/--.*//')   # explicit jar path
  else row=$(awk -F"\t" -v s="$slug" '$1==s' $B/h2h-groups.txt); [[ -z "$row" ]] && { echo "SKIP	dev-$slug	no group row" >> $B/dev-ledger.txt; continue; }
    flags=$(echo "$row" | cut -f2); jars=$(echo "$row" | cut -f3); fi
  ( [[ $flags == *nobasecloth* ]] && export NOBASECLOTH=1
    source <(sed '/^CMDS=/,$d' $B/run-world.sh) >/dev/null 2>&1
    LEDGER=$B/dev-ledger.txt
    run_one "dev-$slug" ${=jars} )
