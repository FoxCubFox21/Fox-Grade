#!/usr/bin/env python3
"""Build the NeoForge test corpus the same way the Fabric one was built: the most-used mods that really ship a
NeoForge build for the source version, minus the ones Fox-Grade declares out of scope (heavyweight rendering)."""
...
OUT = pathlib.Path.home() / "foxgrade-work/nf-corpus"
...
def main():
    ...
    while len(hits) < WANT * 4 and offset < 300:
        page = api("search", facets=[["categories:neoforge"], [f"versions:{SRC}"], ["project_type:mod"]],
