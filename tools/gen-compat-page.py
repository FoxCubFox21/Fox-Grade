...
NOTES = {"rei": "needs cloth-config built for 1.21.1 next to the instance's 26.2 build; Fabric rejects the dependency before either tool runs",
  ...
  "supplementaries": "moonlight's RegSupplier implements Holder, which is a sealed interface in 26.2",
  "amendments": "its third-person renderer mixin extends AgeableListModel, a model class 26.2 removed from the hierarchy",
  ...
  "wthit": "reads ReloadableServerResources' frozen registry access, which 26.2 restructured",
  "chipped": "the game never starts loading the world with it (silent stall after registration)",
  "visual-workbench": "puzzleslib's pack-resources lambda implements a callback whose signature 26.2 changed",
  ...
  "yungs-better-dungeons": "YUNG's structure processor types still reach 26.2's codec dispatch as 1.21 lambdas (cast to MapCodec fails), so registry loading errors and the world never opens (run with YUNG's API present)",
  "yungs-better-jungle-temples": "YUNG's structure processor types still reach 26.2's codec dispatch as 1.21 lambdas (cast to MapCodec fails), so registry loading errors and the world never opens (run with YUNG's API present)",
  "yungs-better-strongholds": "YUNG's structure processor types still reach 26.2's codec dispatch as 1.21 lambdas (cast to MapCodec fails), so registry loading errors and the world never opens (run with YUNG's API present)",
  ...
  "attributefix": "bookshelf's client registry helper hits a null registry entry at init",
