...
launch() {   # $1 = log name
  ...
  while [ $w -lt 240 ]; do
    sleep 6; w=$((w+6))
    pgrep -f quickPlaySingleplayer >/dev/null || break
    grep -q "$READY" "$B/log-rm-$1.log" 2>/dev/null && { sleep 20; break; }
  done
}
reset() {
  pkill -f quickPlaySingleplayer 2>/dev/null; sleep 2
  rm -f $PT/mods/*.jar $PT/mods/*.jar.disabled; for b in $BASE; do cp "$b" $PT/mods/; done
...
rm_run() {
  ...
  launch "$name-A"; pkill -f quickPlaySingleplayer 2>/dev/null; sleep 2
  ...
  launch "$name"
  local v=CRASH; grep -q "$READY" "$B/log-rm-$name.log" && pgrep -f quickPlaySingleplayer >/dev/null && v=PASS
  pkill -f quickPlaySingleplayer 2>/dev/null; sleep 2
