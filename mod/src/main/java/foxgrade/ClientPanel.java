// The client-side panel and autotest, split out of the entrypoint on purpose.
//
// Everything here is written against Minecraft's own classes under their 26.x names. Loading this class on a target
// that runs through intermediary fails before a line of it executes — the JVM resolves the field and method types
// while linking, so a version check inside a method here would never be reached. FoxGradeClient keeps no Minecraft
// type in its own signature and only mentions this class behind that check, which is what makes the check work.
package foxgrade;
  ...
  void install() {
    ...
    ClientTickEvents.END_CLIENT_TICK.register((mc) -> {
      ...
      if (autotestScreen != null && counted == Math.max(1, autotestTicks - 30)) {
        ...
        try {
          Class<?> c = Class.forName(autotestScreen, true, ClientPanel.class.getClassLoader());
          Object s;
